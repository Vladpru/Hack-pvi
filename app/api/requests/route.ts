...
export async function POST(req: Request) {
  ...
  if (!resolvedWarehouseId) {
    ...
    if (bestWh[0]) {
      resolvedWarehouseId = (bestWh[0] as { warehouse_id: number }).warehouse_id
    } else {
      ...
      resolvedWarehouseId = anyWh[0] ? (anyWh[0] as { warehouse_id: number }).warehouse_id : null
    }
  }
  ...
  if (priority === 'critical' || type === 'emergency') {
    ...
    const wName = resolvedWarehouseId
      ? ((await db.execute({ sql: 'SELECT name FROM warehouses WHERE id = ?', args: [resolvedWarehouseId] })).rows[0] as { name: string })?.name ?? 'Unknown'
      : 'Unassigned'
    await db.execute({
